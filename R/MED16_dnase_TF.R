...
TF <- readr::read_csv("tf_targets.csv")
library(dplyr)
TF_abca <- TF %>% filter(target == "ABCA7")
gr_tf_abca7 <- GRanges("chr19", IRanges(start = TF_abca$start,
                                  end = TF_abca$end,
                                  names = TF_abca$TF),
                 strand = "+",
                 score = TF_abca$score)

tf <- append(gr_tf, gr_tf_abca7)
tr_abca7 <- new("track", dat=tf, type="data", format="BED")
...
tiff("images/HiC_MED16_TFs.tiff", height = 45, width = 65, units='cm',
     compression = "lzw", res = 300)
vp <- viewTracks(trackList, gr=gr_1, viewerStyle=viewerStyle,
                 autoOptimizeStyle=TRUE)
addGuideLine(c(1063444, 1043639, 1050875, 1050421, 1046521, 1053524, 870000, 880000), vp=vp, col = "brown")
dev.off()
